...
---
## **v2.3 개발 계획 (Future Development Plan)**

*이제 다음 단계로 아래 기능들의 개발을 진행합니다.*
...
* **(신규) 정산 완료 후 댓글 기능 유지**: 정산 상태가 'archived'(보관/완료)로 변경되어 정산표 자체는 읽기 전용(read-only)이 되더라도, 해당 정산에 대한 댓글은 이전과 동일하게 작성, 수정, 삭제할 수 있도록 기능을 유지합니다. 이를 통해 정산이 끝난 후에도 추가적인 논의나 기록(예: 송금 확인 댓글)이 가능하도록 합니다.
...
* **데이터베이스**:
    ...
    * 댓글의 소프트 딜리트를 위해 `comments` 데이터 구조에 `is_deleted`와 같은 boolean 타입의 컬럼을 추가해야 합니다.
* **API**:
