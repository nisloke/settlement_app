...
*   **원인**: 기존 방식은 이미지 압축 작업이 브라우저의 메인 스레드에서 실행되어, 무거운 압축 작업 중에 UI 렌더링이 차단되는 것이 원인이었습니다.
*   **개선 방안 (Web Worker 도입)**:
    *   이미지 압축 로직을 백그라운드에서 처리하는 전용 **Web Worker (`image-compressor.worker.js`)를 도입**했습니다.
    *   이제 이미지 압축은 메인 UI 스레드와 독립적으로 실행되므로, 압축 중에도 키보드 입력 등 사용자 인터페이스가 지연 없이 부드럽게 동작합니다.
