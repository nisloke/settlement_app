...
1.  **v2.10: 고유 URL 생성 및 라우팅**: 핵심 아키텍처 변경으로, 다른 기능들의 기반이 됩니다.
2.  **v2.8: 소셜 로그인 기능 추가**: 라우팅과 연관된 인증 흐름을 개선하며 사용자 편의성을 높입니다.
3.  **v2.7: 역할 기반 권한 관리 (RBAC)**: 안정적인 아키텍처 위에 세분화된 권한을 구현하는 것이 가장 효율적입니다.
...
---
## **v2.8 개발 계획 (소셜 로그인 기능 추가)**

### **1. 목표**
*   Supabase의 OAuth 기능을 활용하여 구글, 애플, 네이버, 카카오 등 주요 소셜 로그인을 지원합니다. 이를 통해 사용자 편의성을 높이고, 민감한 개인정보(비밀번호)를 직접 저장하지 않아 보안을 강화합니다.

### **2. 구현 방안**
*   **Supabase 설정**: 각 OAuth 제공업체(Google, Apple, Naver, Kakao)의 클라이언트 ID와 시크릿 키를 Supabase 프로젝트에 등록하고, 각 제공업체별 콜백 URL을 설정합니다.
*   **프론트엔드 (React)**:
    *   `Login.jsx` 컴포넌트를 수정하여 각 소셜 로그인(구글, 애플, 네이버, 카카오) 버튼을 추가합니다.
    *   각 버튼 클릭 시, Supabase의 `signInWithOAuth()` 함수를 해당 제공업체(`provider`) 옵션과 함께 호출하도록 구현합니다.
    *   로그인 페이지의 UI를 각 소셜 로그인 아이콘을 포함하여 사용자가 직관적으로 인식할 수 있도록 개선합니다.
*   **콜백 처리 및 프로필 생성**:
    *   OAuth 인증 후 사용자가 애플리케이션으로 리디렉션될 때, 세션 정보가 올바르게 처리되는지 확인합니다.
    *   기존에 구현된 신규 사용자 트리거(`profiles` 테이블에 정보 자동 생성)가 소셜 로그인 시에도 정상적으로 동작하는지 검증하고, 필요시 수정합니다.
--- 
...
*   **개선 방안 (FileReader API로 전환)**:
    ...
    *   이 방식은 이미지 파일을 메모리에 직접 상주시키는 대신 Base64 데이터 URL로 변환하여 사용하므로, 메모리 부하가 적어 UI 반응성이 안정적으로 유지됩니다.

---
## **v2.10 개발 계획 (고유 URL 생성 및 라우팅)**

### **1. 목표**
* 각 정산 페이지(진행 중, 완료 포함)에 대한 고유 URL을 생성하여 직접 접근 및 공유가 가능하도록 합니다.
* 랜딩 페이지를 로그인 페이지로 지정하여 사용자 인증 흐름을 개선합니다.

### **2. 복잡도**
* **중상**: `react-router-dom` 라이브러리 도입, 애플리케이션 구조 변경, 데이터 로딩 로직 수정 등 핵심적인 부분을 수정해야 하므로 복잡도가 높습니다.

### **3. 개발 계획**
1.  **`react-router-dom` 설치**: `npm install react-router-dom` 명령어로 라우팅 라이브러리를 프로젝트에 추가합니다.
2.  **라우터 설정**:
    *   `main.jsx`에서 `App` 컴포넌트를 `<BrowserRouter>`로 감싸 라우팅을 활성화합니다.
3.  **라우트 정의**:
    *   `/`: 랜딩 페이지. 비로그인 시 `Login` 컴포넌트를, 로그인 시 최근 정산 페이지로 리디렉션합니다.
    *   `/settlement/:id`: 특정 정산 페이지. URL의 `:id`를 이용해 해당 정산 데이터를 불러와 `ExpenseTable`과 `CommentSection`을 표시합니다.
4.  **`App.jsx` 리팩토링**:
    *   URL로 `settlementId`를 관리하므로, 기존의 `settlementId` 상태를 제거합니다.
    *   `useParams` 훅을 사용하여 URL의 `id`를 가져오도록 데이터 로딩 로직을 수정합니다.
5.  **네비게이션 업데이트**:
    *   `Sidebar`에서 정산 항목 선택 시, `<Link>` 컴포넌트를 사용하여 `/settlement/:id` 형태의 URL로 이동하도록 수정합니다.
    *   새 정산 생성 후, 해당 정산의 고유 URL로 자동 이동시킵니다.
6.  **인증 및 리디렉션**:
    *   인증되지 않은 사용자가 정산 페이지 접근 시, 로그인 페이지로 리디렉션하는 로직을 구현합니다.
    *   로그인 성공 후, 원래 접근하려던 페이지로 다시 이동시킵니다.